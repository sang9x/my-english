...
import TypingViGame from '@/components/TypingViGame';
import Link from 'next/link';
import { TOPICS, WORDS } from '@/lib/data';

export default async function TypeViPage({ params }: { params: Promise<{ topicId: string }> }) {
    ...
    <div className="page-wrapper" style={{ minHeight: '100dvh' }}>
      ...
      <div className="container-app" style={{ paddingTop: '32px', paddingBottom: '80px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
          ...
          <h1 style={{ fontWeight: 700, fontSize: '1.25rem', color: 'var(--text-primary)', margin: 0 }}>🇻🇳 Gõ tiếng Việt</h1>
        </div>
        {words && words.length >= 1 ? (
          <TypingViGame words={words} topicId={topicId} />
        ) : (
